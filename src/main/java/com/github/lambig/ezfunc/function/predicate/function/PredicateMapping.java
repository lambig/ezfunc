...
public abstract class PredicateMapping {
  ...
  public static <O, P> Mapping<P, Predicate<O>> having(final Mapping<O, P> extraction) {
    return new Mapping<P, Predicate<O>>() {
      @Override
      public Predicate<O> map(final P currentCriteria, final int index) {
  ...
  public static <T> Mapping<Predicate<? super T>, T> firstSatisfierIn(final List<T> list) {

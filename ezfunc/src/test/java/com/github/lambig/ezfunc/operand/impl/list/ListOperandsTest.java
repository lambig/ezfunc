...
public class ListOperandsTest {
	...
	public static class forEach {
		...
		public void elements() {
			// SetUp
			ListOperands<Integer> target = ListOperands.of(1, 2, 3);
			List<Integer> actual = Lists.newArrayList(0);
	...
	public static class sort {
		...
		@Test
		public void with_sequence() {
			// SetUp
			ListOperands<Integer> target = ListOperands.of(1, 2, 3);
			// Exercise
			String actual = target.sort(Lists.newArrayList(3, 1, 2)).map(TO_STRING).reduce(StringReduction.join(""));
			// Verify
			assertThat(actual, is("312"));
		}

		@Test
		public void with_sequence_of_property() {
			// SetUp
			Box<Integer> a = new Box<>(1);
			Box<Integer> b = new Box<>(2);
			Box<Integer> c = new Box<>(3);
			ListOperands<Box<Integer>> target = ListOperands.of(a, b, c);
			// Exercise
			String actual = target.sort(Lists.newArrayList(3, 1, 2), extraction())
					.map(extraction())
					.map(TO_STRING)
					.reduce(StringReduction.join(""));
			// Verify
